...
- Firefox package: `release/dutchmate-firefox-0.6.0.zip`
- Chromium package for Chrome: `release/dutchmate-chrome-0.6.0.zip`
- Chromium package for Edge: use `release/dutchmate-chrome-0.6.0.zip` unless a separate Edge-specific package is created
- GitHub Release Edge asset: `release/dutchmate-edge-0.6.0.zip` (the verified
  Chromium package under an Edge-specific filename)
- GitHub release notes source: `docs/release/notes/v0.6.0.md`
- Published Edge listing: `https://microsoftedge.microsoft.com/addons/detail/dutchmate/plobaccfjjbpfekjomnmmidlmjjdecme`
- Edge submission packet: `docs/release/edge-addon-0.5.1-submission.md` (historical first-publication record)
...
## Current 0.6.0 Artifact Status

Generated by the GitHub Release workflow from the merged `main` source:

- `release/dutchmate-firefox-0.6.0.zip`
- `release/dutchmate-chrome-0.6.0.zip`
- `release/dutchmate-edge-0.6.0.zip`

Older `0.1.x` through `0.5.1` artifacts may still exist locally in `release/`
for reference. Use the `0.6.0` artifacts for the current release.

The Firefox source package is not included in the current `0.6.0` GitHub
release; prepare one only if AMO requests source code.

There is no separate Edge-specific build script. The GitHub release workflow
attaches the verified Chrome Chromium package again under the Edge-specific
filename for the Edge submission.
...
4. Upload `release/dutchmate-firefox-0.6.0.zip`.
...
DutchMate 0.6.0 is approved and publicly available on Edge:
...
5. Upload `release/dutchmate-chrome-0.6.0.zip`.
