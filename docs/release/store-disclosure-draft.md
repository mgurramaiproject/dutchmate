...
```text
DutchMate helps people learn Dutch online by translating text users deliberately select between Dutch, English, and a supported mother tongue such as Telugu. Users can keep local learning items, practise Daily Five and optional Context Missions, use bundled lessons, and save selected single-word translations locally.
```

## Short Description

```text
Learn Dutch online with selected-text translations in Dutch, English, and your language.
```
...
It lets you select a word, phrase, or short sentence on a webpage to see translations between Dutch, English, and a supported mother tongue such as Telugu.
...
```text
DutchMate helps users learn Dutch online by translating deliberately selected webpage text between Dutch, English, and a supported mother tongue.
```
...
```text
DutchMate sends selected website text to https://dutchmate-backend.onrender.com/translate, along with language settings and translation context, so the extension can return translations.
```
...
- **Website content**: selected words, selected phrases, or selected short sentences from webpages.
- **User activity / interaction data**: the selected-text translation action.
- **User settings and local learning data**: source language, target language, selection settings, cache/privacy preferences, and learning items, capped contexts, mastery, lesson progress, rhythm, and saved vocabulary entries stored locally in the browser.
...
```text
DutchMate sends selected website text to https://dutchmate-backend.onrender.com/translate only for translation. The backend sends the text needed for translation to Google Cloud Translation.
...
The extension sends selected website text to https://dutchmate-backend.onrender.com/translate by default for translation requests. The backend uses Google Cloud Translation.
