...
Daily Five and lesson work are focused flows. They hide top-level navigation and provide an explicit Exit action. Lessons show an equal-width `Read | Notice | Practise | Keep` rail directly below the focused header, with the current labelled stage marked in orange. Orange is never the only indication of stage because the stage number and label remain visible.
