...
The automated gate passes for the current implementation: 92 test files / 534 tests, typecheck, Chrome and Firefox builds, release packaging, and whitespace verification. MGurram confirmed the current 005-refinement build looks good in both Firefox and Chrome, completing the lifecycle, accessibility, and popup-geometry gate recorded in `docs/release/manual-testing.md`.

## Firefox feedback follow-up status (2026-07-24)

The popup follow-up adds persistent Today routes for lesson learning and Saved review, replaces pathway chips with readiness plus CEFR-level filters, opens the native browser Save As chooser for Saved Export, highlights each saved Dutch term in its stored context, makes activity totals visually consistent while explaining incomplete legacy history, shows the local-only/no-account boundary in primary views, and makes Telugu phonetics discoverable during answer reveal and in Saved rows. Saved now places the website-selection and local-only guidance together directly beneath its heading, and Lesson filters use compact controls. Import continues to use the native JSON file chooser. The focused popup/session checks, full 92-file / 534-test suite, typecheck, Chrome and Firefox builds, release packaging, whitespace verification, and user-confirmed Chrome/Firefox browser pass are complete.
