...
describe("createLearningClient", () => {
  it("lists, deletes, and practices through the typed learning contract", async () => {
    ...
    await expect(client.recordMissionResult(item.id, "recognition", "got-it", 0)).resolves.toEqual(item);
    await expect(client.removeContext(item.id, { text: "Goede morgen, buur.", addedAt: 1_000 })).resolves.toEqual(item);
    expect(sendMessage).toHaveBeenCalledWith({ type: "dutchmate.learning.delete", payload: { id: item.id } });
    expect(sendMessage).toHaveBeenCalledWith({ type: "dutchmate.learning.removeContext", payload: { itemId: item.id, context: { text: "Goede morgen, buur.", addedAt: 1_000 } } });
  });
});
