...
function renderGrammarAnswerControls(exercise: GrammarExercise): HTMLElement {
...
async function showGrammarOutcome(patternId: GrammarPatternId, exerciseId: string, outcome: "reveal" | "skip"): Promise<void> {
  ...
  const record = grammarRecords[patternId] ?? (grammarPatternId === patternId ? grammarRecord : null);
  if (!record) { grammarOutcome = null; grammarChecked = false; grammarFeedback = { correct: false, message: "Grammar practice is unavailable right now. Try again later." }; render(); return; }
  pending = true; render();
  try {
