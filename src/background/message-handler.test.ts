...
describe("createBackgroundMessageHandler", () => {
  ...
  it("routes verb journey evidence through the revision-checked learning boundary", async () => {
    ...
    const payload = { verbId: "verb.werken" as const, formOrSkillId: "skill.werken.vtt-completed", exerciseFamily: "meaning", exerciseId: "exercise.werken.vtt.meaning", contentVersion: "019-1" as const, result: "correct" as const, expectedEvidenceRevision: 0 };
  ...
  it("routes a Verb Journey Daily Five result through the shared snapshot boundary", async () => {
    ...
    const records = new LearningRecordStore(storage, () => 1_000);
    const handleMessage = createBackgroundMessageHandler({ learningRecords: records, refreshBadge: async () => undefined });
    await send(handleMessage, { type: LEARNING_VERB_JOURNEY_RESULT_MESSAGE, payload: { verbId: "verb.werken", formOrSkillId: "skill.werken.vtt-completed", contentVersion: "019-1", exerciseFamily: "meaning", exerciseId: "exercise.werken.vtt.meaning", result: "incorrect", expectedEvidenceRevision: 0 } });
