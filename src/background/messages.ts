...
export type LearningMessage =
  ...
  | { type: typeof LEARNING_VERB_JOURNEY_RESULT_MESSAGE; payload: { verbId: string; formOrSkillId: string; exerciseFamily: string; exerciseId: string; contentVersion: "015-1"; result: "correct" | "incorrect"; delayedOrRecombined?: boolean; expectedEvidenceRevision: number } };
...
export function isLearningMessage(message: unknown): message is LearningMessage {
  ...
  if (message.type === LEARNING_VERB_JOURNEY_RESULT_MESSAGE) return payload.verbId === "verb.werken" && typeof payload.formOrSkillId === "string" && typeof payload.exerciseFamily === "string" && typeof payload.exerciseId === "string" && payload.contentVersion === "015-1" && (payload.result === "correct" || payload.result === "incorrect") && (payload.delayedOrRecombined === undefined || typeof payload.delayedOrRecombined === "boolean") && typeof payload.expectedEvidenceRevision === "number" && Number.isInteger(payload.expectedEvidenceRevision) && payload.expectedEvidenceRevision >= 0;
