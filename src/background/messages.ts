...
export type LearningMessageResponse = { ok: true; result: { items: LearningItem[] } | { total: number; due: number; new: number; recent: LearningItem[] } | { rhythm: LearningRhythm } | { grammar: GrammarRecord | null } | { grammar: GrammarRecord; snapshot: DailyFiveSnapshot } | { contrast: ContrastRecord | null } | { contrast: ContrastRecord; repairOffer: ImmediateContrastRepairOffer | null } | { contrast: ContrastRecord; snapshot: DailyFiveSnapshot } | { item: LearningItem } | { deleted: true } | { cleared: true } | { backup: LearningBackup } | { importedCount: number; totalCount: number; items: LearningItem[] } | { recorded: true } | { snapshot: DailyFiveSnapshot } | { item: LearningItem; snapshot: DailyFiveSnapshot } | { progress: LessonProgress | null } } | { ok: false; error: string };
