...
export type LearningMessage =
  ...
  | { type: typeof LEARNING_CONTRAST_RESULT_MESSAGE; payload: { packId: ContrastPackId; contentVersion: 1; exerciseId: string; answer?: string; expectedEvidenceRevision: number; outcome?: "reveal" | "skip"; misconceptionCode?: ContrastMisconceptionCode } };
...
export type LearningMessageResponse = { ok: true; result: { items: LearningItem[] } | { total: number; due: number; new: number; recent: LearningItem[] } | { rhythm: LearningRhythm } | { grammar: GrammarRecord | null } | { grammar: GrammarRecord; snapshot: DailyFiveSnapshot } | { contrast: ContrastRecord | null } | { contrast: ContrastRecord; repairOffer: ImmediateContrastRepairOffer | null } | { item: LearningItem } | { deleted: true } | { cleared: true } | { backup: LearningBackup } | { importedCount: number; totalCount: number; items: LearningItem[] } | { recorded: true } | { snapshot: DailyFiveSnapshot } | { item: LearningItem; snapshot: DailyFiveSnapshot } | { progress: LessonProgress | null } } | { ok: false; error: string };
...
export function isLearningMessage(message: unknown): message is LearningMessage {
  ...
  if (message.type === LEARNING_CONTRAST_RESULT_MESSAGE) return payload.packId === "contrast.main_clause_inversion" && payload.contentVersion === 1 && typeof payload.exerciseId === "string" && contrastPack.exercises.some((exercise) => exercise.id === payload.exerciseId) && typeof payload.expectedEvidenceRevision === "number" && Number.isInteger(payload.expectedEvidenceRevision) && payload.expectedEvidenceRevision >= 0 && (payload.outcome === undefined || payload.outcome === "reveal" || payload.outcome === "skip") && (payload.misconceptionCode === undefined || payload.misconceptionCode === "MAIN_CLAUSE_NO_INVERSION") && (payload.outcome !== undefined ? payload.answer === undefined : typeof payload.answer === "string");
