...
export type LearningMessage =
  ...
  | { type: typeof LEARNING_GRAMMAR_RESULT_MESSAGE; payload: { patternId: "a0-zijn-present"; contentVersion: 1; exerciseId: string; answer: string; expectedEvidenceRevision: number } };
...
export function isLearningMessage(message: unknown): message is LearningMessage {
  ...
  if (message.type === LEARNING_GRAMMAR_RESULT_MESSAGE) return payload.patternId === "a0-zijn-present" && payload.contentVersion === 1 && typeof payload.exerciseId === "string" && typeof payload.answer === "string" && typeof payload.expectedEvidenceRevision === "number" && Number.isInteger(payload.expectedEvidenceRevision) && payload.expectedEvidenceRevision >= 0;
